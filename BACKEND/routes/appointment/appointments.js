...
const Appointment = require('../../models/appointment/appointment'); // Adjusted import to match the 
...
router.get('/get/:id', async (req, res) => {
  ...
  try {
    ...
    if (!appointment) {
      ...
      res
        .status(200)
        .json({ message: 'Appointment fetched successfully', appointment }); // Adjusted response message format
    }
  } catch (err) {
    console.log(err);
    res.status(500).json({ error: err.message }); // Adjusted response status and error message
